...
Public Class frmConsultaRucv1
    ...
    Dim afectoRUS As String = String.Empty
    Dim actividadComercioExterior As String = String.Empty
    Dim sisEmi As String = String.Empty
    Dim fechaEmiElec As String = String.Empty
    Dim compElec As String = String.Empty
    ...
    Private Function LeerCaptcha() As Image
        ...
        Try
            ...
            Dim myWebRequest As HttpWebRequest = DirectCast(WebRequest.Create("http://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/captcha?accion=image"), HttpWebRequest) 'http://www.sunat.gob.pe/cl-ti-itmrconsruc/captcha?accion=image&magic=2
    ...
    Private Sub MostrarDatos()
        If Not String.IsNullOrEmpty(nombre) Then
            ...
            txtAfectRUS.Text = afectoRUS.Trim()
            txtActvComExt.Text = actividadComercioExterior.Trim()
            TextBox1.Text = sisEmi.Trim()
            TextBox2.Text = fechaEmiElec.Trim()
            TextBox3.Text = compElec.Trim()
    ...
    Sub BuscaRuc(ByVal numeroRuc As String, ByVal codigoCaptcha As String)
        ...
        Try
            ...
            'Dim myUrl As String = String.Format("http://www.sunat.gob.pe/cl-ti-itmrconsruc/jcrS00Alias?accion=consPorRuc&nroRuc={0}&codigo={1}",
            '                            numeroRuc, codigoCaptcha)
            Dim myUrl As String = String.Format("http://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/jcrS03Alias?accion=consPorRuc&nroRuc={0}",
                                        numeroRuc)
            ...
            If Not estado.Contains("SUSPENSION") Then
                principio = "Condición del Contribuyente:</td> =""bg"" colspan=1>"
                final = "</td>"
                posicion1 = InStr(texto, principio)
            ...
            End If
            '======================= Buscar RUS =======================
            principio = "Afecto al Nuevo RUS: </td> =""bg"" colspan=1>"
            final = "</td>"
            posicion1 = InStr(texto, principio)
            posicion2 = InStr(posicion1 + 44, texto, final)
            posicion1 += 43
            afectoRUS = Mid(texto, posicion1, (posicion2 - posicion1))
            If afectoRUS <> "SI" And afectoRUS <> "NO" Then
                afectoRUS = ""
            End If
            '======================= Buscar Actividad =======================
            principio = "Actividad de Comercio Exterior:</td> =""bg"" colspan=1>"
            final = "</td>"
            posicion1 = InStr(texto, principio)
            posicion2 = InStr(posicion1 + 54, texto, final)
            posicion1 += 53
            actividadComercioExterior = Mid(texto, posicion1, (posicion2 - posicion1))

            '======================= Buscar Sitema de Emisión de Comprobante =======================
            principio = "Sistema de Emisión de Comprobante:</td> =""bg"" colspan=1>"
            final = "</td>"
            posicion1 = InStr(texto, principio)
            posicion2 = InStr(posicion1 + 57, texto, final)
            posicion1 += 56
            sisEmi = Mid(texto, posicion1, (posicion2 - posicion1))

            '======================= Buscar Sitema de Emisor electrónico desde =======================
            principio = "Emisor electrónico desde: </td> =""bg"" colspan=3>"
            final = "</td>"
            posicion1 = InStr(texto, principio)
            posicion2 = InStr(posicion1 + 49, texto, final)
            posicion1 += 48
            fechaEmiElec = Mid(texto, posicion1, (posicion2 - posicion1))

            '======================= Buscar Sitema de Emisor electrónico desde =======================
            principio = "Comprobantes Electrónicos: </td>  =""bg"" colspan=3>"
            final = "</td>"
            posicion1 = InStr(texto, principio)
            posicion2 = InStr(posicion1 + 51, texto, final)
            posicion1 += 50
            compElec = Mid(texto, posicion1, (posicion2 - posicion1))
    ...
    Private Sub frmConsultaRuc_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.pbCaptcha.Image = ObtieneImagenCaptcha() 'LeerCaptcha()
    End Sub

    Private Function ObtieneImagenCaptcha() As Image
        Dim Url As String = String.Empty
        Dim UrlCaptcha As HttpWebRequest = WebRequest.Create("http://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/captcha?accion=image")
        UrlCaptcha.CookieContainer = objCookie
        ServicePointManager.SecurityProtocol = SecurityProtocolType.Ssl3
        UrlCaptcha.Credentials = CredentialCache.DefaultCredentials
        Dim Captcha As WebResponse = UrlCaptcha.GetResponse
        Dim ImagenCaptchaBinario As Stream = Captcha.GetResponseStream
        Return Image.FromStream(ImagenCaptchaBinario)
    End Function
End Class